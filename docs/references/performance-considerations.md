...
4.  **Storage Adapter Choice & `queryRules` Efficiency:**
    ...
    - **Details:**
      ...
      - **`queryRules` Implementation:** This is CRITICAL. The `Storage` interface requires a `queryRules(action, resource)` method. Adapters that efficiently filter rules _at the data source_ (e.g., using SQL `WHERE` clauses with database indexes, like the SQLite and Prisma examples) will vastly outperform adapters that fetch _all_ rules and filter them in application memory (like the basic LocalStorage and Redis examples) when dealing with a large total number of rules.
