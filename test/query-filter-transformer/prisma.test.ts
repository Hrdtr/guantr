...
const mockContext = {
  ...
  name: 'Alice',
  role: 'admin',
} satisfies Omit<MockResourceMap['user'], 'posts'>
    ...
    guantr.setPermissions([
      {
        action: 'read',
        resource: 'post',
        condition: { 'author.role': ["equals", "user"], 'author.active': ["equals", true] },
        inverted: false,
      },
      {
        action: 'read',
        resource: 'post',
        condition: { 'author.role': ["equals", "admin"], 'author.active': ["equals", true] },
        inverted: false,
      },
    ])
