...
describe('matchConditionExpression - every operator', () => {
  const testCases = [
    ...
    { value: [{ id: 1, value: 40 }, { id: 2, value: 50 }, { id: 3, value: 60 }], operand: { value: ['eq', 50] }, expected: false }, // Not all items with value === 50
    { value: [{ id: 1, value: 10 }, { id: 2, value: 20 }], operand: { value: ['eq', 30] }, expected: false }, // No item with value === 30

    // Multiple Conditions
    { value: [{ id: 1, value: 60, status: 'active' }, { id: 2, value: 70, status: 'active' }], operand: { value: ['gt', 50], status: ['eq', 'active'] }, expected: true }, // All items with value > 50 and status === 'active'
    { value: [{ id: 1, value: 60, status: 'active' }, { id: 2, value: 70, status: 'inactive' }], operand: { value: ['gt', 50], status: ['eq', 'active'] }, expected: false }, // Not all items with value > 50 and status === 'active'
    { value: [{ id: 1, name: 'alice', age: 25 }, { id: 2, name: 'bob', age: 30 }], operand: { name: ['eq', 'alice'], age: ['gte', 25] }, expected: false }, // Not all items with name === 'alice' and age >= 25

    // Nested
    { value: [{ id: 1, name: { first: 'John', last: 'Doe' } }, { id: 2, name: { first: 'Alice', last: 'Doe' } }], operand: { name: { last: ['eq', 'Doe'] } }, expected: true }, // All items with name.last === 'Doe'
    ...
    { value: [{ id: 1, value: 10 }, { id: 2, value: 'twenty' }], operand: { value: ['eq', 'twenty'] }, expected: false }, // Not all items with value === 'twenty'
  ]
